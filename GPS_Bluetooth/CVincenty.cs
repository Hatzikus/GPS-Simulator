...
namespace GPS_Bluetooth
{
	class CVincenty
	{
		public CVincenty()
		{
			...
			direct(-37.95103, 144.42487, 54972.271, 306.86816, out dLatitude2, out dLongitude2);

			// -37.652817717441515, 143.92649766821214
			//  37.6528°S,          143.9265°E
		}

		static public double toRadians(double dDegrees)
		{
			return (dDegrees * Math.PI / 180.0);
		}
		static public double toDegrees(double dRadians)
		{
			return (dRadians / Math.PI * 180.0);
		}
		/**
			 ...
			 */
		static public void direct(double dLatitude, double dLongitude, double dDistance, double dInitialBearingRadiant, out double dLatitude2, out double dLongitude2)
		{
		// Quelle: https://github.com/chrisveness/geodesy/blob/master/latlon-ellipsoidal-vincenty.js
		//direct(distance, initialBearing) {
			...
			double φ1 = toRadians(dLatitude);
			double λ1 = toRadians(dLongitude);

			//const α1 = Number(initialBearing).toRadians();
			double α1 = dInitialBearingRadiant; // toRadians(dInitialBearingDegrees);
			...
			} while (Math.Abs(σ-σʹ) > 1e-12 && iIterations < 100);
			...
			dLatitude2 = toDegrees(φ2);
			dLongitude2 = toDegrees(λ2);
...
/*        return {
		 ...
		 * @throws  {EvalError}  Formula failed to converge.
		 */
		static public double inverse(double dLatitude1, double dLongitude1, double dLatitude2, double dLongitude2)
		{
			// Quelle: https://github.com/chrisveness/geodesy/blob/master/latlon-ellipsoidal-vincenty.js
			// inverse(point) {
			...
			double φ1 = toRadians(dLatitude1);
			double λ1 = toRadians(dLongitude1);
			double φ2 = toRadians(dLatitude2);
			double λ2 = toRadians(dLongitude2);
			...
			bool antipodal = Math.Abs(L) > Math.PI/2 || Math.Abs(φ2 - φ1) > Math.PI/2;
			...
			double σ = antipodal ? Math.PI : 0.0;
			double sinσ = 0.0;
			...
			do {
				...
				sinSqσ = Math.Pow(cosU2 * sinλ, 2) + Math.Pow(cosU1 * sinU2 - sinU1 * cosU2 * cosλ, 2);
